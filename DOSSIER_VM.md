...
## 3 septies. ⭐ `curl` ATTEINT le 100.x — par le SOCKS de `tailscaled`, et `NO_PROXY` le cachait (mesuré le 25/09/2026)

*Session du 25/09, VM neuve (canari absent), kernel 6.18.44, Tailscale 1.102.4. Cas d'usage :
le Pi `pxl-matrixled` (`tag:pxl-dev`, API Flask sur :8080), piloté depuis la VM (`tag:pxl-vm`).
Recette rejouable : `pxl-ledmatrix/scripts/claude_tailnet.sh`.*

Le § 3 quinquies et le § 3 sexies posent que **`tailscale nc` est le seul outil qui route le
100.x**. **Incomplet** : `tailscaled` sait exposer un **proxy SOCKS5**, et `curl` passe par lui.

```bash
tailscaled --tun=userspace-networking --state=mem: --socks5-server=localhost:1055   # tâche suivie
curl --noproxy '' --socks5-hostname localhost:1055 http://100.69.91.56:8080/health  # → {"status":"ok"}
```

⇒ plus besoin du pont `127.0.0.1:8099 → tailscale nc` du § 3 quinquies pour un simple client HTTP :
tout outil qui parle SOCKS5 (curl, `ALL_PROXY=socks5h://…`, Python `requests[socks]`) atteint le
tailnet directement.

### 🔴 Le piège : `NO_PROXY` de la VM contient `100.64.0.0/10`

| Commande | Résultat | Pourquoi |
|---|---|---|
| `curl --socks5-hostname localhost:1055 http://100.69.91.56:8080/health` | **timeout** 15 s | `NO_PROXY` couvre la plage CGNAT de Tailscale ; **curl l'applique même à un proxy passé en option** → il ignore le SOCKS et part en direct, vers nulle part |
| idem, **`--noproxy ''`** | **200** `{"status":"ok"}` | `NO_PROXY` neutralisé, la requête entre dans le SOCKS |
| `curl --socks5-hostname localhost:1055 https://pxl-matrixled.<tailnet>.ts.net/health` | **200** | le nom `*.ts.net` n'est pas dans `NO_PROXY` — d'où un 443 qui « passe » quand le 8080 « ne passe pas » |

⚠️ **Ce timeout se lit exactement comme un refus d'ACL** — et la doctrine du § 3 sexies (*« seul
un pendu est un refus d'ACL »*) pousse précisément à cette lecture. Elle reste juste **pour
`tailscale nc`** ; pour tout autre client, **vérifier d'abord que le paquet est entré dans
Tailscale**. Le contrôle décisif est sur le nœud cible : `tailscale debug netmap` →
`PacketFilter` liste les règles **reçues** (sources × ports) — c'est ce qui a disculpé l'ACL ici.

### Trois constats de plus, même séance

1. **`tailscale ssh` refuse les options `-o` d'OpenSSH devant la cible** et **affiche son aide**
   — ce qui se lit comme un échec de connexion. Appelé nu (`tailscale ssh root@pxl-matrixled
   'cmd'`), il passe. *Voir errata 18.*
2. **`tailscale nc` rend la main dès l'EOF de stdin** : « rien reçu » ne prouve pas un port fermé.
   Garder stdin ouvert : `(printf 'GET / HTTP/1.0\r\n\r\n'; sleep 5) | tailscale nc …`.
...
| Joindre un hôte hors allowlist | 403 CONNECT (§3) — *vrai pour Internet ; les nœuds du tailnet, eux, sont joignables (§ 3 bis → 3 septies)* |
...
17. ❌ **« `tailscale nc` est le seul outil qui route le 100.x ; `curl` ne voit pas le
    tailnet. »** (§ 3 quinquies, § 3 sexies, `CLAUDE.md`) **Incomplet, mesuré le 25/09** :
    `tailscaled --socks5-server` + `curl --noproxy '' --socks5-hostname` atteint le 100.x. La
    conclusion venait très probablement du même piège que celui payé le 25/09 : **`NO_PROXY`
    contient `100.64.0.0/10`**, et curl l'applique même à un proxy explicite — il ne tente
    jamais le tunnel. Le 25/09, je l'ai d'abord lu, moi aussi, comme **un refus d'ACL sur le
    8080** ; l'ACL était juste (le `PacketFilter` reçu par le Pi l'a prouvé), et l'indice était
    sous les yeux : le 443 du même nœud passait, **par son nom `*.ts.net`**. **Leçon : un
    timeout n'est pas un refus ; avant d'accuser la politique distante, vérifier que le paquet
    a pris le chemin prévu — ici, c'est une variable d'environnement LOCALE qui mentait.**

18. ❌ **« Le Pi n'a pas Tailscale SSH, seulement OpenSSH. »** (25/09) Conclu parce que
    `tailscale ssh -o … user@nœud` « échouait » — il affichait en fait **son aide** (options `-o`
    refusées) — puis qu'un `ssh` OpenSSH via `tailscale nc` tombait sur `Permission denied`.
    Appelé nu, `tailscale ssh root@pxl-matrixled` passait du premier coup. **Leçon : lire la
    sortie d'un outil avant de la classer en échec — une page d'aide n'est pas un refus.**
